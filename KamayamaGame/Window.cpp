...
void Window::create(Game* game) {
	//@TODO
	//gameがゲーム画面の縦横サイズを保持していてそれによってウィンドウのサイズも変わるようにする
	hwnd = CreateWindowEx(
...
{
	userData.reset();
	userData = move(make_unique<UserData>(game, this));
	SetWindowLongPtr(hwnd, GWLP_USERDATA, (LONG_PTR)userData.get());
}
...
void Window::render(const std::unique_ptr<IGameState>& currentState)
{
	//HDC達を確保する
	PAINTSTRUCT ps;
...
LRESULT CALLBACK Window::WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_TIMER: {
		...
		const unique_ptr<IGameState>& currentState = game->getCurrentState();
		inputManager.update();
		currentState->update();
